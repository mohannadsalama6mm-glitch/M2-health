...
fn map(r: &Row<'_>) -> rusqlite::Result<Customer> {
    Ok(Customer {
        id: r.get(0)?,
        code: r.get(1)?,
        name: r.get(2)?,
...
fn validate_fields(name: &str, phone: &str, email: &str, address: &str, notes: &str) -> Result<(), AppError> {
    if name.trim().is_empty() || name.chars().count() > 180 {
        return Err(invalid("A customer name of 1–180 characters is required."));
    }
    ...
    {
        return Err(invalid("A customer contact field is too long."));
    }
    Ok(())
}
...
pub(crate) fn code_exists(c: &Connection, code: &str, exclude_id: &str) -> Result<bool, AppError> {
    Ok(c.query_row(
        "SELECT count(*) FROM customers WHERE code=?1 AND id<>?2",
...
fn unique_code_on(c: &Connection, base: &str) -> Result<String, AppError> {
    ...
    Err(invalid("Could not allocate a unique customer code. Retry."))
}
pub(crate) fn insert(
    c: &Connection,
    code: &str,
    ...
    c.execute(
        "INSERT INTO customers(id,code,name,phone,email,address,notes) VALUES (?1,?2,?3,?4,?5,?6,?7)",
        params![value, code, name, phone, email, address, notes],
    )?;
    get(c, &value)
}
fn validate_create(input: &CreateCustomerInput) -> Result<(), AppError> {
    validate_fields(&input.name, &input.phone, &input.email, &input.address, &input.notes)
}
pub fn create(db: &AppDb, input: &CreateCustomerInput) -> Result<Customer, AppError> {
...
pub(crate) fn get(c: &Connection, value: &str) -> Result<Customer, AppError> {
    c.query_row(
        &format!("SELECT {COLUMNS} FROM customers WHERE id=?1"),
        [value],
        map,
    )
    .optional()?
    .ok_or_else(|| AppError::new(ErrorCode::NotFound, "Customer not found."))
}
pub fn get_customer(db: &AppDb, value: &str) -> Result<Customer, AppError> {
    id(value)?;
    let c = db.lock()?;
    get(&c, value)
}
fn validate_update(input: &UpdateCustomerInput) -> Result<(), AppError> {
    id(&input.id)?;
    validate_fields(&input.name, &input.phone, &input.email, &input.address, &input.notes)
}
pub(crate) fn update_on(
    ...
    c.execute(
        "UPDATE customers SET name=?1,phone=?2,email=?3,address=?4,notes=?5,updated_at=strftime('%Y-%m-%dT%H:%M:%fZ',max(julianday('now'),julianday(updated_at)+0.001/86400)) WHERE id=?6",
        params![name, phone, email, address, notes, value],
    )?;
    get(c, value)
}
pub fn update(db: &AppDb, input: &UpdateCustomerInput) -> Result<Customer, AppError> {
    validate_update(input)?;
    let mut c = db.lock()?;
    let tx = c.transaction_with_behavior(TransactionBehavior::Immediate)?;
    get(&tx, &input.id)?;
    let customer = update_on(
        &tx,
        &input.id,
        input.name.trim(),
        input.phone.trim(),
...
pub(crate) fn set_active_on(c: &Connection, value: &str, active: bool) -> Result<Customer, AppError> {
    c.execute(
        "UPDATE customers SET is_active=?1,updated_at=strftime('%Y-%m-%dT%H:%M:%fZ',max(julianday('now'),julianday(updated_at)+0.001/86400)) WHERE id=?2",
        params![active, value],
    )?;
    get(c, value)
}
pub fn set_active(db: &AppDb, value: &str, active: bool) -> Result<Customer, AppError> {
    id(value)?;
    let c = db.lock()?;
    set_active_on(&c, value, active)
}
pub fn list(db: &AppDb, q: &CustomerQuery) -> Result<CustomerPage, AppError> {
    ...
    let total = tx.query_row(
        &format!("SELECT count(*) FROM customers {filter}"),
        params![q.is_active, search, pattern],
        |r| r.get(0),
    )?;
    let items = {
        let mut s = tx.prepare(&format!(
            "SELECT {COLUMNS} FROM customers {filter} ORDER BY name COLLATE NOCASE, id LIMIT ?4 OFFSET ?5"
        ))?;
